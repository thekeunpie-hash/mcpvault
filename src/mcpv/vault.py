...
class VaultManager:
    ...
    def install(self, force: bool = False):
        """1. MCP Config 하이재킹 (안전장치 포함)"""
        success = self._hijack_config(force)
        if success:
            """2. 부스팅 스크립트 설치 (하이재킹 성공 시에만)"""
    ...
    def _hijack_config(self, force: bool) -> bool:
        ...
        # [Case 2] 기존 MCP가 1개뿐인 경우 (사용자 요청: 교체 비추천)
        if len(servers) == 1 and not force:
            print(f"⚠️  Only 1 MCP server found: {list(servers.keys())}", file=sys.stderr)
            print("   For simple setups, replacing it with Vault might be unnecessary.", file=sys.stderr)
...
:: [MCP Vault Booster]
:: Optimized launch script for Antigravity generated by mcpv
